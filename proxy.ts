...
export let proxyCredentials: { apiKey: string; apiServerUrl: string } | null =
	null;
export function setProxyCredentials(
	creds: { apiKey: string; apiServerUrl: string } | null,
): void {
...
function mapMessageToHistoryItem(
	m: ChatCompletionRequest["messages"][number],
): ChatHistoryItem {
...
function extractVariantFromProviderOptions(
...
async function lookupCatalogMeta(
	apiKey: string,
	apiServerUrl: string,
	modelUid: string,
): Promise<{ isModelRouter?: boolean; inferenceConfig?: InferenceConfig }> {
	try {
		...
		if (catalog) {
			...
			if (entry)
				return {
					...
					inferenceConfig: entry.inferenceConfig,
				};
		}
	} catch {}
	return {};
...
export function serializeResponseMeta(
	meta: ResponseMeta,
	...
	if (meta.actualModelUid !== undefined)
		out["actual_model_uid"] = meta.actualModelUid;
	...
	try {
		...
		if (
			...
			if (!requestBody.messages || !Array.isArray(requestBody.messages)) {
				res.writeHead(400, { "Content-Type": "application/json" });
				res.end(
					JSON.stringify({ error: { message: "messages must be an array." } }),
				);
				return;
			}
			...
			if (!creds) {
				...
				res.end(
					JSON.stringify({
						error: { message: "Not authenticated. Run /login windsurf first." },
					}),
				);
				return;
			}
			...
			const variantOverride = extractVariantFromProviderOptions(
			...
			const multimodalMessages: ChatHistoryItem[] = requestBody.messages.map(
				(m) => mapMessageToHistoryItem(m),
			);
			...
			const catalogMaxTokens =
				modelCatalogEntry?.maxOutputTokens &&
				modelCatalogEntry.maxOutputTokens > 0
					? modelCatalogEntry.maxOutputTokens
					: 128_000;
			...
			const modelSupportsImages =
				modelCatalogEntry?.features?.supportsImageCaptions !== false;
			...
			if (isStreaming) {
				...
				try {
								...
								choices: [
									{
										...
										delta: {
											tool_calls: [
												{ index: idx, function: { arguments: ev.argsDelta } },
											],
										},
										finish_reason: null,
									},
					...
					if (usage) {
						...
						if (responseMeta)
							usageChunk["_windsurf_meta"] =
								serializeResponseMeta(responseMeta);
				...
				if (collected.length > L4_MAX_RESPONSE_CHARS) {
					collected = truncateHeadTail(
						collected,
						L4_MAX_RESPONSE_CHARS,
						L4_TRUNC_MARKER,
					);
				}
				...
				if (responseMeta)
					resp["_windsurf_meta"] = serializeResponseMeta(responseMeta);
				res.writeHead(200, { "Content-Type": "application/json" });
		...
		if (url.pathname === "/v1/messages" || url.pathname === "/messages") {
			...
			if (!creds) {
				...
				res.end(
					JSON.stringify({
						...
						error: {
							type: "api_error",
							message: "Not authenticated. Run /login windsurf first.",
						},
					}),
				);
				return;
			...
			const catalogMaxTokens =
				catalogEntry?.maxOutputTokens && catalogEntry.maxOutputTokens > 0
					? catalogEntry.maxOutputTokens
					: 128_000;

			// Convert Anthropic messages to ChatHistoryItem[]
			// Anthropic tool history uses content blocks:
			//   assistant turn: [{type:"tool_use", id, name, input}, ...]
			//   user turn (result): [{type:"tool_result", tool_use_id, content}, ...]
			// These must be mapped to ChatHistoryItem.tool_calls / tool_call_id so the
			// proto encoder (#6 tool_call, #7 tool_call_id) can pair them — otherwise the
			// model loses all tool context and repeats calls endlessly.
			const multimodalMessages: ChatHistoryItem[] = [];
			if (anthroBody.system) {
				const sysText =
					typeof anthroBody.system === "string"
						? anthroBody.system
						: anthroBody.system.map((p) => p.text).join("\n");
			...
			for (const m of anthroBody.messages) {
				...
				// Separate tool blocks from text/image content
				const textParts: ChatHistoryItem["content"] = [];
				...
				for (const p of m.content) {
					if (p.type === "text") {
						...
						toolResultId = p.tool_use_id;
						if (typeof p.content === "string") {
							toolResultText = p.content;
				...
				// A user turn carrying only tool_result(s) → role:"tool" with tool_call_id
				if (toolResultId !== undefined) {
					...
					// If there's also text alongside the tool_result, emit it as a user message
					if (textParts.length > 0) {
						multimodalMessages.push({ role: "user", content: textParts });
					}
				} else {
					const item: ChatHistoryItem = {
						role: m.role as ChatHistoryItem["role"],
			...
			const requestedMaxTokens =
				typeof anthroBody.max_tokens === "number" && anthroBody.max_tokens > 0
					? anthroBody.max_tokens
					: catalogMaxTokens;
			...
			if (isStreaming) {
				...
				try {
					...
					if (cachedInputTokens !== undefined)
						anthropicUsage.cache_read_input_tokens = cachedInputTokens;
				...
				if (cachedInputTokens !== undefined)
					anthropicUsage.cache_read_input_tokens = cachedInputTokens;
				if (cacheCreationInputTokens !== undefined)
					anthropicUsage.cache_creation_input_tokens = cacheCreationInputTokens;
...
export function storeResponseMeta(
	responseId: string,
	meta: ResponseMeta,
): void {
...
export function startProxy(
	port: number = WINDSURF_PROXY_PORT,
): Promise<number> {
	if (serverInstance)
		return Promise.resolve((serverInstance.address() as { port: number }).port);
...
export function getChildProxyUrl(): {
	baseUrl: string;
